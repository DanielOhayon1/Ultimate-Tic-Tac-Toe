...
char player_to_char(int player);//switches player value with player name.
...
/*fix:	double board print at first turn.
		...
		winner declared in won sub board.
...
int sub_board_win_row(int board[N][N], int row, int col, int player,int sub_index)
{
	for (int i = row; i < row + 3; i++)
	{
		if (board[i][col] == board[i][col + 1] && board[i][col] == board[i][col + 2] && board[i][col] == PLAYER1)
		{
			printf("Player %c has won in sub - board %d:\n",player_to_char(player),sub_index);
			return PLAYER1;
		}
		else if (board[i][col] == board[i][col + 1] && board[i][col] == board[i][col + 2] && board[i][col] == PLAYER2)
		{
			printf("Player %c has won in sub - board %d:\n", player_to_char(player), sub_index);
			return PLAYER2;
		}
...
int sub_board_win_right_slant(int board[N][N], int col, int row, int player, int sub_index)
{
	if (board[row][col+2] == board[row + 1][col + 1] && board[row][col + 2] == board[row + 2][col] && board[row + 1][col + 1] == PLAYER1)
	{
		printf("Player %c has won in sub - board %d:\n", player_to_char(player), sub_index);
		return PLAYER1;
	}
	else if (board[row][col+2] == board[row + 1][col + 1] && board[row][col + 2] == board[row + 2][col] && board[row + 1][col + 1] == PLAYER2)
	{
		printf("Player %c has won in sub - board %d:\n", player_to_char(player), sub_index);
		return PLAYER2;
	}
...
int sub_board_win_left_slant(int board[N][N], int col, int row, int player, int sub_index)
{
	if (board[row][col] == board[row + 1][col + 1] && board[row][col] == board[row + 2][col + 2] && board[row][col] == PLAYER1)
	{
		printf("Player %c has won in sub - board %d:\n", player_to_char(player), sub_index);
		return PLAYER1;
	}
	else if (board[row][col] == board[row + 1][col + 1] && board[row][col] == board[row + 2][col + 2] && board[row][col] == PLAYER2)
	{
		printf("Player %c has won in sub - board %d:\n", player_to_char(player), sub_index);
		return PLAYER2;
	}
...
int sub_board_win(int board[N][N],int player, int sub_index)
{
	...
	win_col = sub_board_win_col(temp, row, col, player, sub_index);
	win_row = sub_board_win_row(temp, row, col, player, sub_index);
	win_left_slant = sub_board_win_left_slant(temp, row, col, player, sub_index);
	win_right_slant = sub_board_win_right_slant(temp, row, col, player, sub_index);
	if (win_row == PLAYER1 || win_col == PLAYER1 || win_right_slant == PLAYER1 || win_left_slant == PLAYER1)
	{
		return PLAYER1;
	}
	if (win_row == PLAYER2 || win_col == PLAYER2 || win_right_slant == PLAYER2 || win_left_slant == PLAYER2)
	{
		return PLAYER2;
	}
	else
